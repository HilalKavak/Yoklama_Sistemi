...
namespace bseu_yoklama_sistemi1
{
    public partial class Form9 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                string query = "SELECT id FROM dbo.Ogrenci_giris WHERE kullanici_adi = @username AND sifre = @password";
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    try
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                Program.CurrentUser = reader["id"].ToString(); // 'id' sütununu alır ve string'e dönüştürür
